...
const standaloneApp = fs.existsSync(path.join(standaloneRootToUse, "server.js"))
  ? standaloneRootToUse
  : (fs.existsSync(path.join(standaloneRootToUse, appFolderName))
      ? path.join(standaloneRootToUse, appFolderName)
      : path.join(standaloneRootToUse, "app"));
if (!fs.existsSync(standaloneApp)) {
  console.error("❌ Next.js standalone build not found under .next/standalone");
  console.error(`Expected either .next/standalone/server.js, .next/standalone/${appFolderName}/, or .next/standalone/app/`);
  process.exit(1);
}
